...
const Home: NextPage = () => {
  ...
  const addToFirebase = (values: any) => {
    ...
    set(ref(db, "teachers/" + userId), { id: userId, ...values });
  };

  const checkDuplicityOnDatabase = (values: any) => {
    let duplicates = false;
    const db = getDatabase(app);
    const topUserPostsRef = query(ref(db, "teachers"));
        ...
        <main className={styles.main}>
          <div className={styles.profileContent}>
            ...
            >
              ...
              <div>
                {profile?.subjects?.map((tag: string, key: number) => (
                  <Tag
                    key={key}
                    color={tag == profile?.masterSubject ? "red" : "volcano"}
                  >
                    {tag}
                  </Tag>
                ))}
              ...
              >
                ...
                <Form.Item name="subjects">
                  <Select
                    ...
                    mode="tags"
                    style={{ width: "100%" }}
                    onChange={(e) => setProfile({ ...profile, subjects: e })}
                    tokenSeparators={[","]}
                  ></Select>
                </Form.Item>
                <Form.Item name="masterSubject">
                  <Select
                    placeholder="Matéria Principal"
                    onChange={(e) =>
                      setProfile({ ...profile, masterSubject: e })
                    }
                  >
                    {profile.subjects?.map((tag: any, key: any) => (
